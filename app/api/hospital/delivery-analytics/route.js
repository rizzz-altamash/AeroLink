...
export async function GET(req) {
  try {
    ...
    // FIXED: Get both incoming and outgoing deliveries for this hospital
    const deliveries = await Delivery.find({
      $or: [
        // Outgoing deliveries (hospital is sender)
        { 'sender.hospitalId': session.user.hospitalId },
        // Incoming deliveries (hospital is recipient)
    ...
    // FIXED: Properly identify incoming vs outgoing
    const incomingDeliveries = deliveries.filter(d => {
      // Check if this hospital is the recipient
      return d.recipient?.hospitalId?.toString() === session.user.hospitalId.toString() || 
             d.metadata?.deliveryType === 'incoming';
    });
    
    const outgoingDeliveries = deliveries.filter(d => {
      // Check if this hospital is the sender AND it's not marked as incoming
      return d.sender?.hospitalId?.toString() === session.user.hospitalId.toString() && 
             d.metadata?.deliveryType !== 'incoming';
    });
    ...
    // Prepare chart data with fixed filtering
    const chartData = await prepareChartData(deliveries, timeRange, startDate, session.user.hospitalId);
...
// FIXED: Helper function to prepare chart data with proper incoming/outgoing identification
async function prepareChartData(deliveries, timeRange, startDate, hospitalId) {
  const chartData = [];
  const now = new Date();
  ...
  if (timeRange === 'today') {
    ...
    for (let hour = 0; hour < 24; hour++) {
      ...
      // Properly filter incoming vs outgoing
      const incomingCount = hourDeliveries.filter(d => 
        d.recipient?.hospitalId?.toString() === hospitalId.toString() || 
        d.metadata?.deliveryType === 'incoming'
      ).length;
      
      const outgoingCount = hourDeliveries.filter(d => 
        d.sender?.hospitalId?.toString() === hospitalId.toString() && 
        d.metadata?.deliveryType !== 'incoming'
      ).length;
    ...
    for (let i = 0; i < days; i++) {
      ...
      // Properly filter incoming vs outgoing
      const incomingCount = dayDeliveries.filter(d => 
        d.recipient?.hospitalId?.toString() === hospitalId.toString() || 
        d.metadata?.deliveryType === 'incoming'
      ).length;
      
      const outgoingCount = dayDeliveries.filter(d => 
        d.sender?.hospitalId?.toString() === hospitalId.toString() && 
        d.metadata?.deliveryType !== 'incoming'
      ).length;
