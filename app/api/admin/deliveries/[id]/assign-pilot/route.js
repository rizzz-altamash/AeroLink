...
//     const { pilotId, droneId } = await req.json();
...
//       return NextResponse.json({ 
//         error: 'Drone cannot accept this delivery',
//         reason: drone.status !== 'available' ? 'Drone not available' : 
//                drone.health.batteryLevel <= 20 ? 'Low battery' :
//                drone.specifications.maxPayload < delivery.package.weight ? 'Exceeds payload capacity' :
//                'Maintenance required'
//       }, { status: 400 });
//     }

//     // Assign pilot and drone to delivery
...
//       status: 'assigned',
//       timestamp: new Date(),
//       notes: `Assigned to pilot ${pilot.name} with drone ${drone.registrationId}`
//     });
...
//     // Update drone status
//     await drone.startDelivery(delivery._id);
...
//       'specifications.maxPayload': { $gte: delivery.package.weight }
//     }).select('_id registrationId model specifications.maxPayload health.batteryLevel');
