        ...
        <div className="min-h-screen p-4 sm:p-6 lg:p-8">
          ...
          <div className="relative mb-6 sm:mb-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">Pending Approvals</h1>
                <p className="text-sm sm:text-base text-gray-400">Review and approve delivery requests from medical staff</p>
              </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
            <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-yellow-500/20">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 bg-yellow-500/20 rounded-lg flex items-center justify-center">
                  <AlertIcon className="w-4 h-4 sm:w-6 sm:h-6 text-yellow-400" />
                </div>
                <div>
                  <p className="text-xl sm:text-2xl font-bold text-white">{pendingDeliveries.length}</p>
                  <p className="text-xs sm:text-sm text-gray-400">Total Pending</p>
                </div>
              </div>
            </div>

            <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-red-500/20">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 bg-red-500/20 rounded-lg flex items-center justify-center">
                  <AlertIcon className="w-4 h-4 sm:w-6 sm:h-6 text-red-400" />
                </div>
                <div>
            ...
            <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-orange-500/20">
              <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl p-3 sm:p-4 border border-gray-500/20">
              <div className="flex items-center gap-2 sm:gap-3">
                <div className="w-8 h-8 sm:w-10 sm:h-10 bg-cyan-500/20 rounded-lg flex items-center justify-center">
                  <CheckIcon className="w-4 h-4 sm:w-6 sm:h-6 text-cyan-400" />
                </div>
                <div>
                  <p className="text-xl sm:text-2xl font-bold text-white">
                    {pendingDeliveries.filter(d => d.package?.urgency === 'routine').length}
                  </p>
                  <p className="text-xs sm:text-sm text-gray-400">Routine</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Filter Tabs */}
          <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl p-3 sm:p-4 border border-yellow-500/20 mb-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-2">
              <FilterIcon className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 sm:mr-2" />
              ...
              {filter !== 'all' && filteredDeliveries.length > 1 && (
                <button
                  onClick={() => handleBulkApprove(filter)}
                  className="w-full sm:w-auto sm:ml-auto px-3 py-1.5 sm:px-4 sm:py-2 bg-green-600/20 hover:bg-green-600/30 text-green-400 text-xs sm:text-sm rounded-lg transition-all mt-2 sm:mt-0"
                >
                  Approve All {filter}
                </button>
              )}
          ...
          {/* Pending Approvals Grid */}
          <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl p-4 sm:p-6 border border-yellow-500/20">
            {loading ? (
              <div className="flex items-center justify-center py-12">
                <svg className="animate-spin h-8 w-8 text-yellow-500" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
...
function PendingApprovalCard({ delivery, onReview }) {
  ...
  return (
    <div className={`border rounded-xl p-3 sm:p-4 ${urgencyColors[delivery.package?.urgency || 'routine']} hover:scale-[1.02] transition-all flex flex-col h-full`}>
      <div className="flex items-start justify-between mb-3">
        <div className="min-w-0 flex-1 mr-2">
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-sm sm:text-base text-white font-semibold truncate">{delivery.orderId}</h3>
            <span className={`text-xs px-2 py-0.5 rounded-full ${
              isIncoming 
                ? 'bg-blue-500/20 text-blue-400' 
                : 'bg-orange-500/20 text-orange-400'
          ...
          <p className="text-gray-400 text-xs sm:text-sm truncate">{delivery.package?.type}</p>
        </div>
        <span className={`text-xs sm:text-sm font-medium ${urgencyTextColors[delivery.package?.urgency || 'routine']}`}>
          {delivery.package?.urgency?.toUpperCase()}
        </span>
      </div>
      ...
      <div className="space-y-1.5 sm:space-y-2 mb-3 sm:mb-4 flex-1">
        <div className="text-xs sm:text-sm">
          <span className="text-gray-400">Requested by:</span>
          <span className="text-gray-300 ml-1 sm:ml-2 truncate block sm:inline">
            {isIncoming
              ? delivery.metadata?.orderedBy?.name || delivery.sender?.userId?.name || 'Hospital Staff'
              : delivery.sender?.userId?.name || 'Unknown'}
          </span>
        ...
        <div className="text-xs sm:text-sm">
          <span className="text-gray-400">Created:</span>
          <span className="text-gray-300 ml-1 sm:ml-2 text-xs">{new Date(delivery.createdAt).toLocaleString()}</span>
        </div>
      </div>
      
      <button
        onClick={onReview}
        className="w-full py-2 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-400 rounded-lg transition-all font-medium text-sm mt-auto"
      >
        Review & Approve
      </button>
    </div>
...
function ApprovalModal({ delivery, onApprove, onReject, onClose, rejectionReason, setRejectionReason }) {
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
      <div className="bg-gray-900 rounded-2xl p-4 sm:p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">Review Delivery Request</h2>
        ...
        <div className="space-y-3 sm:space-y-4">
          <div>
            <label className="text-gray-400 text-xs sm:text-sm block mb-2">Rejection Reason (if rejecting)</label>
            <textarea
              value={rejectionReason}
              onChange={(e) => setRejectionReason(e.target.value)}
          ...
          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
            ...
            <button
              onClick={onClose}
              className="sm:px-6 py-2.5 sm:py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg font-semibold transition-all text-sm sm:text-base"
            >
              Cancel
            </button>
          </div>
