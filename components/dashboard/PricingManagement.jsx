    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 sm:mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 flex items-center gap-2 sm:gap-3">
          <DollarIcon className="w-6 h-6 sm:w-8 sm:h-8 text-purple-400" />
          Pricing Management
        </h1>
        <p className="text-sm sm:text-base text-gray-400">Configure delivery pricing and charges</p>
      </div>

      {/* Current Configuration */}
      <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-purple-500/20 mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
          ...
          <div className="flex flex-wrap gap-2 w-full sm:w-auto">
            <button
              onClick={() => setEditMode(!editMode)}
              className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-purple-600/20 hover:bg-purple-600/30 text-purple-400 rounded-lg transition-all text-sm sm:text-base"
            >
              {editMode ? 'Cancel' : 'Create New'}
            </button>
            <button
              ...
              className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm sm:text-base"
            >
              <HistoryIcon className="w-4 h-4" />
              History
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-medium text-white mb-2 sm:mb-3">Basic Settings</h3>
            
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Configuration Name</label>
              <input
                type="text"
                value={formData.name}
                onChange={(e) => handleInputChange('name', e.target.value)}
                className="w-full px-3 sm:px-4 py-2 bg-gray-800 rounded-lg text-white focus:ring-2 focus:ring-purple-500 text-sm sm:text-base"
                placeholder="e.g., Holiday Season 2024"
              />
            </div>
          ...
          {/* Urgency Multipliers */}
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-medium text-white mb-2 sm:mb-3">Urgency Multipliers</h3>
            
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Routine (1x)</label>
              <input
                type="number"
                value={formData.urgencyMultipliers.routine}
                onChange={(e) => handleInputChange('urgencyMultipliers.routine', e.target.value)}
            ...
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Urgent</label>
              <input
                type="number"
                value={formData.urgencyMultipliers.urgent}
                onChange={(e) => handleInputChange('urgencyMultipliers.urgent', e.target.value)}
            ...
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Emergency</label>
              <input
                type="number"
                value={formData.urgencyMultipliers.emergency}
                onChange={(e) => handleInputChange('urgencyMultipliers.emergency', e.target.value)}
          ...
          {/* Time-based Multipliers */}
          <div className="space-y-3 sm:space-y-4">
            <h3 className="text-base sm:text-lg font-medium text-white mb-2 sm:mb-3">Time-based Multipliers</h3>
            
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Peak Hour Multiplier</label>
              <input
                type="number"
                value={formData.peakHourMultiplier}
                onChange={(e) => handleInputChange('peakHourMultiplier', e.target.value)}
            ...
            <div>
              <label className="block text-xs sm:text-sm text-gray-400 mb-1">Weekend Multiplier</label>
              <input
                type="number"
                value={formData.weekendMultiplier}
                onChange={(e) => handleInputChange('weekendMultiplier', e.target.value)}
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:justify-between">
          <button
            onClick={() => currentConfig && showPricePreview(currentConfig._id)}
            className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm sm:text-base"
          >
          ...
          <button
            onClick={handleSave}
            className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all flex items-center justify-center gap-2 text-sm sm:text-base"
          >
      ...
      {/* Current Estimates */}
      <div className="bg-gray-900/50 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-purple-500/20 mb-6 sm:mb-8">
        <h2 className="text-lg sm:text-xl font-semibold text-white mb-3 sm:mb-4">Price Estimates</h2>
      ...
      {showHistory && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 max-w-full sm:max-w-4xl w-full max-h-[85vh] sm:max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">Pricing History</h2>
              <button
                onClick={() => setShowHistory(false)}
            ...
            <div className="space-y-3 sm:space-y-4">
              {history.map((config) => (
                <div key={config._id} className={`p-3 sm:p-4 rounded-lg sm:rounded-xl border ${
                  config.isActive ? 'border-purple-500 bg-purple-500/10' : 'border-gray-700 bg-gray-800/50'
                }`}>
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div>
                      <h3 className="text-white font-medium text-sm sm:text-base">{config.name}</h3>
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button
                        onClick={() => showPricePreview(config._id)}
                        className="flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 bg-gray-700 hover:bg-gray-600 text-white rounded-lg transition-all text-xs sm:text-sm"
                      >
                        Preview
                      </button>
                      {!config.isActive && (
                        <button
                          onClick={() => handleActivate(config._id)}
                          className="flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm"
                        >
                          <ActivateIcon className="w-3 h-3 sm:w-4 sm:h-4" />
                          Activate
                        </button>
                      )}
                    </div>
      ...
      {showPreview && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-gray-900 rounded-xl sm:rounded-2xl p-4 sm:p-6 max-w-full sm:max-w-4xl w-full max-h-[85vh] sm:max-h-[80vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">Price Preview</h2>
              <button
                onClick={() => setShowPreview(false)}
                className="text-gray-400 hover:text-white transition-colors p-1"
              >
            ...
            <div className="space-y-3 sm:space-y-4">
              {previews.map((preview, index) => (
                <div key={index} className="bg-gray-800/50 rounded-lg sm:rounded-xl p-3 sm:p-4">
                  <h3 className="text-white font-medium mb-2 sm:mb-3 text-sm sm:text-base">{preview.scenario}</h3>
