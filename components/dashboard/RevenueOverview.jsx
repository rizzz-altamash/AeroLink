...
//       a.href = url;
//       a.download = `revenue-report-INR-${timeRange}-${new Date().toISOString().split('T')[0]}.${format}`;
//       a.click();
...
// Indian Number System Formatting Functions
...
// For large display numbers (like total revenue)
const formatLargeINR = (amount) => {
  if (amount >= 10000000) {
    return (
      <span>
        ₹{(amount/10000000).toFixed(2)}
        <span className="text-xs ml-1 text-gray-400">Cr</span>
      </span>
    );
  }
  if (amount >= 100000) {
    return (
      <span>
        ...
        <span className="text-xs ml-1 text-gray-400">L</span>
      </span>
    );
  }
        ...
        <div className="bg-gray-800/30 rounded-xl p-3 sm:p-4">
          <div className="text-center mb-3 sm:mb-4">
            ...
            <div className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-purple-400 to-purple-600 bg-clip-text text-transparent">
              {formatLargeINR(revenueData.metrics.total)}
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {formatIndianCurrency(revenueData.metrics.total, 0)}
            </p>
          </div>
          ...
          <div className="space-y-1.5 sm:space-y-2">
            {revenueData.urgencyBreakdown.map((item, index) => (
              <div 
                key={index} 
                className="flex items-center justify-between p-1.5 sm:p-2 rounded hover:bg-gray-700/30 transition-colors cursor-pointer group"
              >
                <div className="flex items-center gap-2 flex-1 min-w-0">
                  <div 
                    className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full flex-shrink-0 shadow-sm" 
          ...
          <div className="mt-3 sm:mt-4 pt-2 sm:pt-3 border-t border-gray-700">
            <div className="flex items-center justify-between text-xs sm:text-sm">
              <span className="text-gray-400">Average</span>
              <span className="text-white font-medium">
                {formatCompactINR(revenueData.metrics.total / Math.max(revenueData.urgencyBreakdown.length, 1))}
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-xl max-h-200 overflow-y-auto rounded-2xl p-4 sm:p-6 border border-purple-500/20 hover:border-purple-500/30 transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4 mb-4 sm:mb-6">
        <MetricCard
          title="Total Revenue"
          value={formatCompactINR(revenueData.metrics.total)}
          change={revenueData.metrics.growth}
          icon={TrendingUpIcon}
          fullValue={formatIndianCurrency(revenueData.metrics.total)}
        />
        <MetricCard
          title="Average"
          value={formatCompactINR(revenueData.metrics.average)}
          subtitle="Per delivery"
          fullValue={formatIndianCurrency(revenueData.metrics.average)}
        />
        <MetricCard
          title="Today"
          value={formatCompactINR(revenueData.metrics.todayRevenue)}
          subtitle="So far"
          fullValue={formatIndianCurrency(revenueData.metrics.todayRevenue)}
        />
        <MetricCard
          title="Forecast"
          value={formatCompactINR(revenueData.forecast.nextPeriod || 0)}
          subtitle={`Next ${timeRange}`}
          fullValue={formatIndianCurrency(revenueData.forecast.nextPeriod || 0)}
        />
      </div>
            ...
            {revenueData.hospitalRevenue.slice(0, 5).map((hospital, index) => (
              <div key={index} className="flex items-center justify-between group">
                ...
                <div className="text-right">
                  <span className="text-xs text-white font-medium">{formatCompactINR(hospital.revenue)}</span>
                  <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
                    {formatIndianCurrency(hospital.revenue, 0)}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="bg-gray-800/30 rounded-xl p-3 sm:p-4">
          ...
          <div className="space-y-1.5 sm:space-y-2">
            <div className="flex items-center justify-between group">
              ...
              <div className="text-right">
                <span className="text-xs text-orange-400 font-medium">
                  {formatCompactINR(revenueData.deliveryTypeSplit.outgoing || 0)}
                </span>
                <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
                  {formatIndianCurrency(revenueData.deliveryTypeSplit.outgoing || 0, 0)}
                </p>
              </div>
            </div>
            <div className="flex items-center justify-between group">
              ...
              <div className="text-right">
                <span className="text-xs text-blue-400 font-medium">
                  {formatCompactINR(revenueData.deliveryTypeSplit.incoming || 0)}
                </span>
                <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
                  {formatIndianCurrency(revenueData.deliveryTypeSplit.incoming || 0, 0)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5 sm:space-y-2">
            {revenueData.peakHours.slice(0, 3).map((hour, index) => (
              <div key={index} className="flex items-center justify-between group">
                <span className="text-xs text-gray-300">{hour.hour}:00</span>
                <div className="text-right">
                  <span className="text-xs text-white font-medium">{formatCompactINR(hour.revenue)}</span>
                  <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
                    {formatIndianCurrency(hour.revenue, 0)}
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="space-y-1.5 sm:space-y-2">
            {revenueData.distanceRevenue.map((range, index) => (
              <div key={index} className="flex items-center justify-between group">
                <span className="text-xs text-gray-300">{range.range}</span>
                <div className="text-right">
                  <span className="text-xs text-white font-medium">{formatCompactINR(range.revenue)}</span>
                  <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
                    {formatIndianCurrency(range.revenue, 0)}
                  </p>
                </div>
              </div>
            ))}
...
// Metric Card Component
function MetricCard({ title, value, change, subtitle, icon: Icon, fullValue }) {
  const isPositive = change > 0;
  
  return (
    <div className="bg-gray-800/30 rounded-xl p-3 sm:p-4 group cursor-pointer hover:bg-gray-800/40 transition-all">
      <div className="flex items-center justify-between mb-1 sm:mb-2">
        <p className="text-gray-400 text-xs sm:text-sm">{title}</p>
        {Icon && <Icon className={`w-3 h-3 sm:w-4 sm:h-4 ${isPositive ? 'text-green-400' : 'text-red-400'}`} />}
      </div>
      ...
      {fullValue && (
        <p className="text-xs text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
          {fullValue}
        </p>
      )}
      {change !== undefined && (
